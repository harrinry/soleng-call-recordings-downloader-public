        ...
        return new Promise((resolve, reject) => {
            ...
            fs.exists(audioFilePath, (exist) => {
                if(exist) {
                    ...
                    stream.on('end', () => {
                      if (!stream.complete) {
                        reject('Stream closed for Recording #' + id + " recording will not be downloaded.");
                      } else {
                        console.log('Stream ended - Going to write recording with id ' + id + ' to file...');
                        let file = fs.createWriteStream(audioFilePath);
                        
                        file.write(data, (error) => {
                            if (error) {
                              console.error('Failed to save Recording #' + id + " on the filesystem due to " + error);
                            }
                        });

                        file.on('finish', () => {
                          console.debug('Finished writing to file.');
                          file.close();
