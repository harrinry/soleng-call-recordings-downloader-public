    ...
    return new Promise((resolve, reject) => {
      ...
      let request = transport.request(options, (res) => {
        ...
        if (res.statusCode >= 400) {
          ...
          let cookie = getHeaderParameter(res.headers, 'Set-Cookie');

          let recList = new RecordingsList();
          let response = ''

          if(cookie) {
            recList.cookie = cookie;
            ...
            this._logger.error('No recording cookie on headers');
  
            reject(new Error('No recording cookie on headers'));
          }
        }
      });
